...
VecXad InertialAlignmentConstraint::constraintFunction(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input,
                                                       const VecXad& parameters) const
{
  ...
  {
    Vec3ad angular_vel = kinematics_ptr_->getAngularVelocityCppAd(state, input);
    Vec3ad angular_acc = kinematics_ptr_->getAngularAccelerationCppAd(state, input);

    Mat3ad ddC_we = dC_dtt<ocs2::ad_scalar_t>(C_we, angular_vel, angular_acc);
...
ocs2::ad_scalar_t InertialAlignmentCost::costFunction(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input,
                                                      const VecXad& parameters) const
{
  Mat3ad C_we = kinematics_ptr_->getOrientationCppAd(state);
  Vec3ad linear_acc = kinematics_ptr_->getAccelerationCppAd(state, input);
...
void InertialAlignmentCostGaussNewton::initialize(size_t nx, size_t nu, size_t np, const std::string& modelName,
                                                  const std::string& modelFolder, bool recompileLibraries, bool verbose)
{
  ...
  ad_interface_ptr_.reset(new ocs2::CppAdInterface(ad_func, 1 + nx + nu, np, modelName, modelFolder));

  if (recompileLibraries)
  {
    ad_interface_ptr_->createModels(ocs2::CppAdInterface::ApproximationOrder::First, verbose);
  }
  else
  {
    ad_interface_ptr_->loadModelsIfAvailable(ocs2::CppAdInterface::ApproximationOrder::First, verbose);
  }
}

ocs2::scalar_t InertialAlignmentCostGaussNewton::getValue(ocs2::scalar_t time, const VecXd& state, const VecXd& input,
                                                          ...
                                                          const ocs2::PreComputation&) const
{
  ...
  const VecXd f = ad_interface_ptr_->getFunctionValue(tapedTimeStateInput, getParameters(time, target));
  return 0.5 * settings_.cost_weight * f.dot(f);
}

ocs2::ScalarFunctionQuadraticApproximation
InertialAlignmentCostGaussNewton::getQuadraticApproximation(ocs2::scalar_t time, const VecXd& state, const VecXd& input,
                                                            const ocs2::TargetTrajectories& target,
                                                            const ocs2::PreComputation&) const
{
  ...
  const VecXd e = ad_interface_ptr_->getFunctionValue(tapedTimeStateInput, params);
...
VecXad InertialAlignmentCostGaussNewton::function(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input,
                                                  const VecXad& parameters) const
{
  Mat3ad C_we = kinematics_ptr_->getOrientationCppAd(state);
  Vec3ad linear_acc = kinematics_ptr_->getAccelerationCppAd(state, input);

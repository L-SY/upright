...
namespace upright {
    ...
    public:
        ...
        ocs2::ScalarFunctionQuadraticApproximation getQuadraticApproximation(
            ...
            // dimension of zero.
            auto approximation = ocs2::ScalarFunctionQuadraticApproximation(state.rows(), 0);
            approximation.setZero(state.rows(), 0);

            // Linear approximations of position and orientation error
            const auto eePosition = kinematics_ptr_->getPositionLinearApproximation(state).front();
            const auto eeOrientationError = kinematics_ptr_->getOrientationErrorLinearApproximation(state,{desired_pose.second}).front();
...
//    Error when build but it is not been used now.

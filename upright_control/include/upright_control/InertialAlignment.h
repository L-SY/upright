...
inline std::ostream& operator<<(std::ostream& out, const InertialAlignmentSettings& settings)
{
  out << "cost_enabled = " << settings.cost_enabled << std::endl
      << "constraint_enabled = " << settings.constraint_enabled << std::endl
      << "use_angular_acceleration = " << settings.use_angular_acceleration << std::endl
      << "cost_weight = " << settings.cost_weight << std::endl
      << "contact_plane_normal = " << settings.contact_plane_normal.transpose() << std::endl
...
class InertialAlignmentConstraint final : public ocs2::StateInputConstraintCppAd
{
public:
  InertialAlignmentConstraint(const ocs2::PinocchioEndEffectorKinematicsCppAd& kinematics,
    ...
    , dims_(dims)
  {
    initialize(dims.x(), dims.u(), 0, "inertial_alignment_constraint", "/tmp/ocs2", recompile_libraries, true);
  }
  ...
  InertialAlignmentConstraint* clone() const override
  {
    ...
    return new InertialAlignmentConstraint(*kinematics_ptr_, settings_, gravity_, dims_, false);
  }
  ...
  {
    return getNumConstraints(0);
  }

protected:
  VecXad constraintFunction(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input,
                            const VecXad& parameters) const override;

private:
  InertialAlignmentConstraint(const InertialAlignmentConstraint& other) = default;
...
public:
  ...
  InertialAlignmentCost(const ocs2::PinocchioEndEffectorKinematicsCppAd& kinematics,
                        const InertialAlignmentSettings& settings, const Vec3d& gravity,
                        const OptimizationDimensions& dims, bool recompile_libraries)
    : kinematics_ptr_(kinematics.clone()), settings_(settings), gravity_(gravity), dims_(dims)
  {
    initialize(dims.x(), dims.u(), 0, "inertial_alignment_cost", "/tmp/ocs2", recompile_libraries, true);
  }

  InertialAlignmentCost* clone() const override
  {
    return new InertialAlignmentCost(*kinematics_ptr_, settings_, gravity_, dims_, false);
  }

protected:
  ocs2::ad_scalar_t costFunction(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input,
                                 const VecXad& parameters) const override;
...
public:
  ...
  InertialAlignmentCostGaussNewton(const ocs2::PinocchioEndEffectorKinematicsCppAd& kinematics,
                                   const InertialAlignmentSettings& settings, const Vec3d& gravity,
                                   const OptimizationDimensions& dims, bool recompile_libraries)
    : kinematics_ptr_(kinematics.clone()), settings_(settings), gravity_(gravity), dims_(dims)
  {
  ...
  {
    return new InertialAlignmentCostGaussNewton(*kinematics_ptr_, settings_, gravity_, dims_, false);
  }
  ...
  void initialize(size_t nx, size_t nu, size_t np, const std::string& modelName, const std::string& modelFolder,
                  bool recompileLibraries, bool verbose);

  ocs2::scalar_t getValue(ocs2::scalar_t time, const VecXd& state, const VecXd& input,
                          const ocs2::TargetTrajectories& target, const ocs2::PreComputation&) const;

  ocs2::ScalarFunctionQuadraticApproximation getQuadraticApproximation(ocs2::scalar_t time, const VecXd& state,
                                                                       const VecXd& input,
                                                                       const ocs2::TargetTrajectories& target,
                                                                       const ocs2::PreComputation&) const;

protected:
  VecXad function(ocs2::ad_scalar_t time, const VecXad& state, const VecXad& input, const VecXad& parameters) const;

private:
  InertialAlignmentCostGaussNewton(const InertialAlignmentCostGaussNewton& other) = default;

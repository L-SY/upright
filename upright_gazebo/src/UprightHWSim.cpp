#include "upright_gazebo/UprightHWSim.h"
#include <gazebo_ros_control/gazebo_ros_control_plugin.h>

namespace upright {
bool UprightHWSim::initSim(
  ...
  bool ret = DefaultRobotHWSim::initSim(robot_namespace, model_nh, parent_model,
                                        urdf_model, transmissions);
  // Joint interface
  //  TODO : (4yang) Add hubridJoint for MIT motor
  //  registerInterface(&hybridJointInterface_);
  //  std::vector<std::string> names = ej_interface_.getNames();
  //  for (const auto &name : names) {
  //    hybridJointDatas_.push_back(
  //        HybridJointData{.joint_ = ej_interface_.getHandle(name)});
  //    HybridJointData &back = hybridJointDatas_.back();
  //    hybridJointInterface_.registerHandle(
  //        HybridJointHandle(back.joint_, &back.posDes_, &back.velDes_,
  //        &back.kp_,
  //                          &back.kd_, &back.ff_));
  //    cmdBuffer_.insert(
...
void UprightHWSim::readSim(ros::Time time, ros::Duration period) {
...
void UprightHWSim::writeSim(ros::Time time, ros::Duration period) {
  //  for (auto joint : hybridJointDatas_) {
  ...
  //
  //    while (!buffer.empty() &&
  //           buffer.back().stamp_ + ros::Duration(delay_) < time) {
  //      buffer.pop_back();
  //    }
  ...
  //    joint.joint_.setCommand(
  //        cmd.kp_ * (cmd.posDes_ - joint.joint_.getPosition()) +
...
PLUGINLIB_EXPORT_CLASS(upright::UprightHWSim, gazebo_ros_control::RobotHWSim)
GZ_REGISTER_MODEL_PLUGIN(
    gazebo_ros_control::GazeboRosControlPlugin) // Default plugin

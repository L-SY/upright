...
public:
  RosWebotsDeliver(ros::NodeHandle nh) {
    ...
    if (nh.getParam("/robot_def/motors_name", motorsNameList) &&
        nh.getParam("/robot_def/pos_sensor_name", posSensorNameList)) {
      if (motorsNameList.getType() == XmlRpc::XmlRpcValue::TypeArray) {
        for (int i = 0; i < motorsNameList.size(); ++i) {
          if (motorsNameList[i].getType() == XmlRpc::XmlRpcValue::TypeString) {
            robotMotorName_.push_back(
                static_cast<std::string>(motorsNameList[i]));
            robotSensorName_.push_back(
                static_cast<std::string>(posSensorNameList[i]));
          } else {
  ...
  void initDeliver(ros::NodeHandle nh) {
    observation_ = nh.subscribe<ocs2_msgs::mpc_flattened_controller>(
        "/mobile_manipulator_mpc_policy", 10,
        &RosWebotsDeliver::MPCPolicyCallback, this);
    rosMotorCmdSub_ = nh.subscribe<std_msgs::Float64MultiArray>(
        "/ros_motor_cmd", 10, &RosWebotsDeliver::rosMotorCmdCallback, this);
    webotsMotorPosPub_ =
        nh.advertise<std_msgs::Float64MultiArray>("/webots_motor_pos", 1);
    webotsMotorVelPub_ =
        nh.advertise<std_msgs::Float64MultiArray>("/webots_motor_vel", 1);
    webotsMotorTorPub_ =
        nh.advertise<std_msgs::Float64MultiArray>("/webots_motor_tor", 1);
  ...
  void enablePositionSensor(webots::Robot *robot, int samplingPeriod = 1000) {
    for (int i = 0; i < robotSensorName_.size(); ++i) {
      webots::PositionSensor *ps =
          robot->getPositionSensor(robotSensorName_[i]);
  ...
  getMotorAllState(webots::Robot *robot, const std::string &motor_name) {
  ...
  std::unordered_map<std::string, double> getMotorAllState(webots::Robot *robot,
                                                           int index) {
    std::unordered_map<std::string, double> stateMap;
  ...
  std::vector<double> getMotorPositions(webots::Robot *robot,
                                        bool print = false) {
  ...
  enum CommandType { POSITION, VELOCITY, TORQUE, INVALID };

  bool setMotorCommands(webots::Robot *robot, std::vector<double> command_value,
                        CommandType command_type) {
    bool is_command_size_correct = (int)command_value.size() == motorNum_;
    bool is_command_type_valid = command_type == POSITION ||
  ...
  void
  MPCPolicyCallback(const ocs2_msgs::mpc_flattened_controller::ConstPtr &data) {
    if (mpc_policy_.inputTrajectory.empty()) {
      ...
      mpc_policy_.inputTrajectory.begin()->value =
          data->inputTrajectory.begin()->value;
  }

  void velocityModeUpdate(webots::Robot *robot) {
    pubWebotsMotorState(robot);
    setMotorCommands(robot, rosMotorCmd_,

...
public:
  ...
  inline void computerInverseDynamics(std::vector<hardware_interface::JointStateHandle> jnt_states);
  inline void computerInverseDynamics(std::vector<double> jnt_pos, std::vector<double> jnt_vel, std::vector<double> jnt_tau);
...
bool PinocchioInterface::init(ros::NodeHandle &dynamics_nh)
{
  ...
  else
  {
    ...
    num_hw_joints_ = (int)model_.joints.size() - 1; // -1 is for remove the "universe"
  }
...
void PinocchioInterface::computerInverseDynamics(std::vector<hardware_interface::JointStateHandle> jnt_states)
{
...
void PinocchioInterface::computerInverseDynamics(std::vector<double> jnt_pos, std::vector<double> jnt_vel, std::vector<double> jnt_tau)
{
  for (int i = 0; i < (int)tau_.size(); ++i) {
    q_(i) = jnt_pos[i];
    v_(i) = jnt_vel[i];

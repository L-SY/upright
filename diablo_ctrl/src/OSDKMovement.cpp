...
#include "diablo_ctrl/OSDKMovement.h"
...
uint8_t Movement_Ctrl::obtain_control(uint16_t timeout_ms)
{
  if (this->in_control())
    return 5;
  if (vehicle->telemetry->status.ctrl_mode &
      ((1 << (1 + vehicle->telemetry->id * 2)) | (1 << (5 + vehicle->telemetry->id * 2))))
    exit(0);  // control is taken by virtual RC

  Header header;
  header.data.LEN = sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Movement_Ctrl_Request_t) + OSDK_MISC_SIZE;
  ...
  while (ack != 0x0002)
  {
    ...
    uint8_t result = vehicle->hal->serialSend_ack(header.data, ack, OSDK_CONTROL_SET, OSDK_CTRL_AUTHORIZE_ID, &req,
  ...
  idle_buffer = true;
  printf("Now you are in SDK Handle Movement control\n");
  // abort();
  return 0;
}
...
uint8_t Movement_Ctrl::release_control()
{
  ...
  vehicle->hal->serialSend(header.data, OSDK_CONTROL_SET, OSDK_CTRL_AUTHORIZE_ID, &req,
                           sizeof(OSDK_Movement_Ctrl_Request_t));
...
uint8_t Movement_Ctrl::EmergencyBrake(void)
{
  ...
  {
    uint8_t result = vehicle->hal->serialSend_ack(header.data, ack, OSDK_CONTROL_SET, OSDK_EMERGENCY_BRAKE_ID,
...
uint8_t Movement_Ctrl::SendMovementModeCtrlCmd()
{
  ...
  while (ack != 0x0000)
  {
    uint8_t result = vehicle->hal->serialSend_ack(header.data, ack, OSDK_CONTROL_SET, OSDK_MOVEMENT_CTRL_MODE_ID,
...
uint8_t Movement_Ctrl::SendMovementCtrlCmd()
{
  ...
  return vehicle->hal->serialSend(header.data, OSDK_CONTROL_SET, OSDK_MOVEMENT_CTRL_ID, &ctrl_data,
                                  sizeof(OSDK_Movement_Ctrl_t));
}
...
uint8_t Movement_Ctrl::SendTransformUpCmd()
{
  ...
  while (ack != 1)
  {
    uint8_t result = vehicle->hal->serialSend_ack(header.data, ack, OSDK_CONTROL_SET, OSDK_TRANSFORM_ID,

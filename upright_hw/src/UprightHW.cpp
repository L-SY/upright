...
void UprightHW::write(const ros::Time & /*time*/,
...
bool UprightHW::setupTopic(ros::NodeHandle &nh) {
  ...
  webotsMotorPosSub_ = nh.subscribe<std_msgs::Float64MultiArray>(
      "/webots_motor_pos", 1, &UprightHW::webotMotorPosCallback, this);
  webotsMotorVelSub_ = nh.subscribe<std_msgs::Float64MultiArray>(
      "/webots_motor_vel", 1, &UprightHW::webotMotorVelCallback, this);
  webotsMotorTorSub_ = nh.subscribe<std_msgs::Float64MultiArray>(
      "/webots_motor_tor", 1, &UprightHW::webotMotorTorCallback, this);
}
...
bool UprightHW::setupJoints() {
  ...
  for (const auto &joint : robotMotorName_) {
    hardware_interface::JointStateHandle state_handle(
        joint, &jointData_[joint_index].pos_, &jointData_[joint_index].vel_,
        &jointData_[joint_index].tau_);
    jointStateInterface_.registerHandle(state_handle);
    hardware_interface::JointHandle joint_handle(
        state_handle, &jointData_[joint_index].tauDes_);

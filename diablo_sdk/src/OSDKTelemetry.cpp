...
#include "diablo_sdk/OSDKTelemetry.h"
...
uint8_t Telemetry::Calibration(OSDK_Offline_Calibration_Cmd_t calibration_cmd)
{
  ...
  {
    uint8_t result =
        vehicle->hal->serialSend_ack(header.data, ack, OSDK_INIT_SET, OSDK_OFFLINE_CALIBRATION_ID, &calibration_cmd, 2);
    if (result)
      return result;
    if (ack == 0x0001)
    {
...
uint8_t Telemetry::configUpdate(const bool save)
{
  ...
  Header header;
  header.data.LEN = sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Set_Push_Data_Freq_t) + OSDK_MISC_SIZE;
  header.data.SESSION = 1;
  header.data.ACK = 1;
  header.data.SEQ = vehicle->hal->serial_getSeq();
  header.append_crc();

  vehicle->hal->serialSend(header.data, OSDK_INIT_SET, OSDK_SET_PUSH_DATA_FREQ_ID, (uint8_t*)&msg,
                           sizeof(OSDK_Set_Push_Data_Freq_t));
...
{
  Header header;
  header.data.LEN = sizeof(OSDK_Uart_Header_t) + sizeof(float) + OSDK_MISC_SIZE;
  header.data.SESSION = 0;
  header.data.ACK = 0;
  header.data.SEQ = vehicle->hal->serial_getSeq();
  header.append_crc();

  vehicle->hal->serialSend(header.data, OSDK_INIT_SET, OSDK_SET_MAX_SPEED_ID, &max_speed, sizeof(float));
}

uint32_t Telemetry::calculatebitrate()
{
  return (sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Push_Data_Flag_t) + sizeof(OSDK_Push_Data_Status_t) + 4) * 10 *
             (bit_r[0] & 0x0000FFFF) +
         (sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Push_Data_Flag_t) + sizeof(OSDK_Push_Data_Quaternion_t) + 4) * 10 *
             (bit_r[1] & 0x0000FFFF) +
         (sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Push_Data_Flag_t) + sizeof(OSDK_Push_Data_XYZ_t) + 4) * 10 *
             (bit_r[2] & 0x0000FFFF) +
         (sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Push_Data_Flag_t) + sizeof(OSDK_Push_Data_XYZ_t) + 4) * 10 *
             (bit_r[3] & 0x0000FFFF) +
         (sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Push_Data_Flag_t) + sizeof(OSDK_Push_Data_RC_t) + 4) * 10 *
             (bit_r[4] & 0x0000FFFF) +
         (sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Push_Data_Flag_t) + sizeof(OSDK_Push_Data_Power_t) + 4) * 10 *
             (bit_r[5] & 0x0000FFFF) +
         (sizeof(OSDK_Uart_Header_t) + sizeof(OSDK_Push_Data_Flag_t) + sizeof(OSDK_Push_Data_Motor_t) + 4) * 10 *
             (bit_r[6] & 0x0000FFFF);
}
...
{
  Header header;
  header.data.LEN = sizeof(OSDK_Uart_Header_t) + sizeof(uint32_t) + OSDK_MISC_SIZE;
  header.data.SESSION = 2;
  header.data.ACK = 1;
  header.data.SEQ = vehicle->hal->serial_getSeq();
  header.append_crc();
  ...
  {
    result =
        vehicle->hal->serialSend_ack(header.data, ack, OSDK_INIT_SET, OSDK_CONNECTION_CHECK_ID, &key, sizeof(uint32_t));
...
void Telemetry::SerialHandle(void)
{
  while (true)
  {
    ...
    OSDK_Push_Data_Flag_t* flag =
        (OSDK_Push_Data_Flag_t*)(vehicle->hal->serialWaitRXDataS(lock, OSDK_DATA_SET, OSDK_PUSH_DATA_ID));
    if (flag == NULL)
    {
    ...
    if (flag->quaternion)
    {
      ...
      if (log_flag & 1 << TOPIC_QUATERNION)
        quaternionLog(quaternion);
    }
    if (flag->accl)
    {

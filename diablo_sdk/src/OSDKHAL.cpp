...
#include "diablo_sdk/OSDKHAL.h"
#include "diablo_sdk/OSDKHeader.h"

using namespace std;
using namespace DIABLO::OSDK;

void HAL::serialPackData(const OSDK_Uart_Header_t& header, const uint8_t cmd_set, const uint8_t cmd_id,
...
void* HAL::serialWaitRXDataS(std::unique_lock<std::mutex>& lock, const uint8_t cmd_set, const uint8_t cmd_id)
{
  ...
  // return null pointer if does not receive anything from serial port for more than 1 second or the received packet
  // does not pass CRC16 verification
  if (!serial_rx_data_cond.wait_for(lock, timeout, std::bind(&DIABLO::OSDK::HAL::verifyRXType, this, cmd_set, cmd_id)))
    return NULL;
...
uint8_t HAL::serialSend_ack(const OSDK_Uart_Header_t& header, uint16_t& ack, const uint8_t cmd_set,
                            const uint8_t cmd_id, const void* data, const uint32_t data_len)
{
  ...
  static const chrono::duration<int, milli> timeout(100);
  {
    ...
    if (!serial_rx_ack_cond.wait_for(lock, timeout, std::bind(&DIABLO::OSDK::HAL::verifyRXType, this, cmd_set, cmd_id)))
    {
      return 2;
    }
    ack = this->getACK();
  ...
  {
    {
      ...
      if (serial_tx_idle && !serial_tx_cond.wait_for(lock, timeout, [this](void) { return !serial_tx_idle; }))
      {
        serial_tx_idle = true;
        continue;
      }
...
uint8_t HAL_Pi::serialSend(const OSDK_Uart_Header_t& header, const uint8_t cmd_set, const uint8_t cmd_id,
                           const void* data, const uint32_t data_len)
{
  static const chrono::duration<int, milli> timeout(100);
  unique_lock<mutex> lock(serial_tx_mtx);
...
void HAL_Pi::RXMonitorProcess(void)
{
  ...
  while (true)
  {
    ...
    while ((int)mySerial.Available() < (int)sizeof(OSDK_Uart_Header_t) - 1)
      usleep(byte_micro);
    ...
    while ((int)mySerial.Available() < (int)data_len)
      usleep(byte_micro);

...
#define OSDK_MISC_SIZE 4                                       // 8bit cmd size, 8bit cmd id, 16bit total packet CRC
#define OSDK_DATA_POS_OFFSET (sizeof(OSDK_Uart_Header_t) + 2)  // header length + 16bit header crc16
...
#define OSDK_SET_MAX_SPEED_ID 0x3F
 ...
 *       SESSION: session ID, 0: Sender doesn't need ACKs. 1:Sender needs ACKs but can be tolerated. 2:Sender needs ACKs
...
typedef struct
{
  ...
  uint8_t
      SESSION : 5;  // session ID, 0: Sender doesn't need ACKs. 1:Sender needs ACKs but can be tolerated. 2:Sender needs ACKs.*
...
typedef struct
{
  ...
  uint8_t yaw_ctrl_mode : 1;     // 0: control angle    1: control angular velocity
  uint8_t split_ctrl_mode : 1;   // 0: control angle    1: control angular velocity
  uint8_t pitch_ctrl_mode : 1;   // 0: control angle    1: control angular velocity
  uint8_t roll_ctrl_mode : 1;    // 0: control angle    1: control angular velocity
  uint8_t height_ctrl_mode : 1;  // 0: control height   1: control vertical velocity
} __attribute__((packed)) OSDK_Movement_Ctrl_Mode_t;

/**
 * @brief Movement control data (send speed or angle directly)
 * @note Detail, check the webpage
 */
typedef struct
{
  float forward;
 ...
 *       timeout_act: Action after drop connection (1 bit), 1:Automatically switches to real remote control, 0 No
 * switches，execute disconnect program \n timeout_ms: automatic release control authority after timeout ms (15 bit) \n
 */
typedef struct
{

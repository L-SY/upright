...
#include "OnboardSDKUartProtocol.h"
#include "VulcanSerial/SerialPort.h"
...
public:
   ...
   * @note NON-API FUNCTION
   */
  void serialPackData(const OSDK_Uart_Header_t& header, const uint8_t cmd_set, const uint8_t cmd_id, const void* data,
   ...
   */
  virtual uint8_t serialSend(const OSDK_Uart_Header_t& header, const uint8_t cmd_set, const uint8_t cmd_id,
   ...
   */
  uint8_t serialSend_ack(const OSDK_Uart_Header_t& header, uint16_t& ack, const uint8_t cmd_set, const uint8_t cmd_id,
   ...
   */
  void* serialWaitRXDataS(std::unique_lock<std::mutex>& lock, const uint8_t cmd_set, const uint8_t cmd_id);
...
public:
   ...
   *         1: multiplexer wait timeout(most possible reason is bandwidth full) \n
   */
  uint8_t serialSend(const OSDK_Uart_Header_t& header, const uint8_t cmd_set, const uint8_t cmd_id, const void* data,
                     const uint32_t data_len);
